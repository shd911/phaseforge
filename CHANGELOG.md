...
## b141.36 (2026-09-16) — частоты 352.8/384 кГц, отсчёты до 1024K

### Расширенные наборы экспорта (b141.35)
- Частота дискретизации: добавлены 352.8 и 384 кГц. В Rust правок не
  потребовалось — проверка проекта и так принимала 8000..768000, ограничение
  жило только в списке выпадающего меню.
- Отсчёты: добавлены 512K и 1024K (×2 и ×4 от прежнего потолка 256K).
  Потолок `fir::MAX_TAPS` поднят 2^18 → 2^20. Он был продуктовым, а не
  аппаратным: `vDSP_create_fftsetupD` отдаёт рабочую radix-2 схему вплоть до
  log2n=23, а самое большое преобразование при 1024K отсчётах — 2^21 (окно
  строится на удвоенной длине), ~18 мс по замеру.
- Три сообщения об ошибке, где потолок был вписан числом, теперь читают саму
  константу — разъехаться с воротами больше не могут.
- Цена на максимуме: импульс уходит в интерфейс как JSON, 20.2 МБ / 304 мс
  при 1024K против 5.2 МБ / 76 мс при 256K. Рост линейный, обрыва нет.
- Приёмка: границы `taps_valid`; LR4 на 1024K/384 кГц с проверкой конвенции
  задержки N/2; LR4 на 352.8 кГц с точкой −6 дБ; кепстральный путь на
  1024K/384 кГц; сверка обоих списков с пределами Rust во фронте.

### Окно экспорта на аналитическом пути (b141.36)
- Аналитический каскад никогда не читал настройку окна: импульс строится
  прогоном дельты через биквады, гасится только последние 5 % отсчётов
  собственным спадом. Замер: импульс побитово одинаков для Rectangular,
  Hann, Blackman, Blackman-Harris, FlatTop, Kaiser, Dolph-Chebyshev.
  Выпадающее меню при этом было активным и выглядело работающим.
- Теперь на вкладке Export меню Win гаснет с пометкой «не применяется»,
  когда выбранная полоса идёт этим путём; подсказка называет случаи, где
  окно действительно работает — линейная фаза, гауссы и Бессель, subsonic,
  измеренные таргеты, и заметнее всего при коротких Taps и резких переходах
  таргета.
- Условие не вычисляется в интерфейсе заново: диспетчер сообщает путь,
  которым реально пошёл, и значение доезжает до вкладки. Второй предикат
  рядом с `pick_fir_route` разъехался бы.
- Приёмка: побитовое совпадение выхода для семи окон (если каскад когда-нибудь
  начнёт окно использовать — упадёт тест, а не подсказка станет враньём);
  во фронте — совпадение объявленного пути с фактически вызванной командой.

## b141.34 (2026-09-05) — аудит 9 векторами, рабочий диапазон 30 кГц

### Рабочий диапазон (b141.34)
- Верхняя граница инструмента поднята с 20 до 30 кГц одной константой
  (`F_MAX_WORK`, ~35 захардкоженных 20000 заменены): лимиты ввода
  кроссовера/PEQ/зон исключения, диапазон PEQ-оптимизации (с потолком
  Nyquist·0.95 частоты экспорта), сетка расширения, ось X графика,
  разбиение диапазона на полосы, стандартная сетка таргета в Rust.
- Полоса согласования уровней (авто-референс, выравнивание в Σ, метрики
  экспорта) намеренно осталась 20 кГц (`F_MAX_REF`) — иначе спад ВЧ
  твитера тянул референс вниз.
- Биквад PEQ клампит частоту ниже Nyquist.

### Аудит 2026-09-05 (b141.26–b141.33), полный отчёт `docs/audit-2026-09-05.md`
- **DSP — min-phase для графика** (b141.27): `compute_minimum_phase` брал
  Nyquist из последней точки сетки и держал магнитуду плоской ниже неё —
  Gaussian/subsonic min-phase полосы расходились с экспортируемым WAV на
  30–100°. Теперь истинный Nyquist из sample rate, лог-лог экстраполяция за
  сетку, 2^17 FFT. Аналитический тест BW2 (≤3° на обеих сетках).
- **DSP — знак фазы баффл-степа** (b141.26) был перевёрнут (−20° на f3
  вместо +20°), утекал в оценку задержки NF/FF merge.
- **Надёжность** (b141.26): невалидный `export_taps` из .pfproj ронял FFT
  внутри async-команды и вешал IPC навсегда — валидация taps (степень
  двойки 32..262144) и измерений (длины/NaN/сортировка) при загрузке
  проекта и версии; атомарная запись .pfproj (tmp + fsync + rename).
- **Кэш вычислений** (b141.27, b141.33): ключ строился от живого
  store-proxy, данные читались после await — правка во время генерации +
  Undo отдавали отравленные записи. Снэпшот полосы до ключа (в `untrack`).
  Σ на вкладке SPL больше не клонирует полосы и попадает в кэш.
- **Версии проекта** (b141.32): снэпшот копирует файлы измерений и NF/FF
  в `snapshots/<id>/` — повторный импорт файла с тем же именем больше не
  переписывает старые версии.
- **IR/Step измерения** (b141.32): считается на той же расширенной сетке,
  что и corrected — Step измерения без плато у DC; в Σ per-band кривые из
  единого evaluator'а, ~200 строк дублирующего DSP удалены из графика.
- **UI** (b141.28, b141.30): ошибки открытия/сохранения/импорта показываются
  toast'ом (были unhandled rejections); диалог кроссовера в dB/oct (был
  сырой порядок, LR 1/3/5/6/7 снапились к 4); валидация ввода (0 Hz в
  кроссовере, `|| 24` в настройках FIR, лимит gain PEQ); Tauri ask вместо
  window.confirm; Escape в диалогах; ~30 подписей переведены; smoothing /
  SR / taps из проекта валидируются по наборам дропдаунов.
- **Мёртвый код** (b141.29, −545 строк): ветка MixedPhase+gaussian,
  legacy hybrid FIR, недостижимая вкладка «step», команда
  `compute_delay_info`, дубли helpers; `needAutoFit` перенесён из App.tsx
  в store (цикл импортов).
- **Производительность** (b141.31): дедупликация одинаковых вычислений в
  полёте, rAF-троттлинг IPC при drag PEQ, параллельные Hilbert-вызовы,
  LTO в release-профиле.
- **Качество** (b141.28): `tsc --noEmit` 18 → 0, добавлен в CI и `npm test`.
- Закрыто как by design: per-band peak-нормировка WAV (гэйны выставляются
  в хосте по «Нормировка»). Открыто: `compute_impulse` до 8 MB JSON,
  инверсия слоёв lib→stores→components.

## b141.25 (2026-08-19 … 2026-06-11) — пост-аудит 2026-08-19

- **Custom-кроссовер** (b141.17): порядок фильтра учитывается в IIR-каскаде
  экспорта (был всегда 2-й порядок).
- **Наклон и полки таргета** (b141.18) попадают в экспортируемый FIR
  (маршрут Cepstral при tilt/shelf).
- **Конвенция задержек** (b141.19): флаг `delay_positive_is_late`
  сохраняется — задержки не переворачивались при каждой загрузке.
  Экспорт возвращает фактическую задержку WAV, предупреждение при недоборе.
- **Экспорт на сетке превью** (b141.20).
- **Окно Дольфа-Чебышева** (b141.21): математически верное и O(n log n).
- **Устаревание PEQ** (b141.22) учитывает sample rate, linear phase, subsonic.
- **IR/Step** (b141.23–25): ось времени выводится, а не передаётся; тоггли
  отображения рисуются из кэша без пересчёта.
